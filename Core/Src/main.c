...
void SysTick_Init(uint32_t tick_freq_hz)
{
  uint32_t sysclk = HAL_RCC_GetHCLKFreq(); // Get system clock frequency

  // Set reload value
  uint32_t reload = (sysclk / tick_freq_hz) - 1;

  if (reload > SysTick_LOAD_RELOAD_Msk) {
    // Reload value too big
    while (1);
  }
...
{
    while (!(USART1->ISR & USART_ISR_TXE));  // Wait for TDR to be empty
    USART1->TDR = c;
